...
| `feedback_entity` | entity id | — | An `input_number` holding a subjective comfort bias in degrees. Wire it to voice ("I'm feeling warm" → decrement) and the occupant becomes a first-class input: the bias shifts the fused target directly, clamped to ±2°. It is applied **after** the comfort-band clamp and is bounded only by the wrapped device's own range — the whole point of the channel is that the band is wrong for this person right now, so clamping the bias back into the band made it saturate at the edge and do nothing. The entity's advertised `min_temp`/`max_temp` widen by the same ±2° so the target can never fall outside its own limits, and a change to this helper re-evaluates immediately instead of waiting for the next `sample_interval`. Pair with a decay automation so a feeling expressed at noon doesn't still govern at midnight. |
